...
### 게임플레이
![게임_플레이](https://github.com/user-attachments/assets/7b31c79e-d9dc-494c-b6fc-72e37600d194)
...
이 프로젝트를 통해 가장 크게 얻은 성과는 **플랫폼에 의존하지 않는 게임 로직 구조를 직접 설계하고 구현한 경험**입니다.

초기 콘솔 기반으로 게임을 만들었지만, 이후 SDL2 환경으로 확장할 때 **core 로직을 거의 수정하지 않고 그대로 재사용할 수 있었습니다.**
입력·렌더링·사운드를 인터페이스로 분리해두었기 때문에, 플랫폼 레이어만 교체하면 동작하는 구조를 만들 수 있었습니다.

이 경험을 통해 단순히 “동작하는 코드”를 만드는 것과,
“변경에 강한 구조”를 만드는 것의 차이를 실제로 체감했습니다.
설계 단계에서 투자한 시간이 이후 확장 과정에서 얼마나 큰 비용 절감을 가져오는지 직접 확인한 의미 있는 성과였습니다.

### 기술적 학습

- ✅ C++ 실전 개발 경험 (스마트 포인터, RAII 기반 자원 관리, constexpr 활용)
- ✅ 게임 개발 기초 (프레임 기반 물리, 충돌 처리, 상태 기계 구현)
- ✅ Clean Architecture 원칙 적용 (추상화, 의존성 역전, 플랫폼 분리)
- ✅ SDL2 렌더링·입력·오디오 시스템 활용