...
**Building Breaker**는 하늘에서 끝없이 떨어지는 건물을 검으로 베어 파괴하는 액션 게임입니다. 플레이어는 사무라이 캐릭터를 조작하여 좌우로 이동하며 건물을 공격하고, 떨어지는 건물을 방어하며, 적절한 타이밍에 필살기를 활용해야 합니다. 시간이 지날수록 건물의 낙하 속도가 빨라지고 내구도가 높아지며, 최대한 오래 생존하며 높은 점수를 달성하는 것이 목표입니다.
